...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const slots = await db
		...
		.where(and(eq(availabilitySlots.isActive, true), gte(availabilitySlots.date, todayStr)))
		.groupBy(availabilitySlots.id)
		.orderBy(availabilitySlots.date, availabilitySlots.startTime);

	const firstAvailable = slots.find((s) => {
		const remaining = s.maxCapacity - Number(s.bookedSeats);
		return remaining > 0 && s.date <= toStr;
	});
